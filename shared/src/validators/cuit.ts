  ...
  for (const pattern of patterns) {
    ...
    for (const match of matches) {
      ...
      const contextBefore = text.slice(
        Math.max(0, position - contextSize),
        position,
      );
      const contextAfter = text.slice(
        position,
        Math.min(text.length, position + contextSize),
      );
      ...
      const hasSectionSeparator = /cliente:|factura\s+n[ºo°]:/i.test(
        contextAfter.slice(0, 160),
      );

      let hasStrongEmitterIndicator = false;
      if (
        strongEmitterKeywords.some((keyword) =>
          contextBeforeClose.includes(keyword),
        )
      ) {
        hasStrongEmitterIndicator = true;
      } else if (
        strongEmitterKeywords.some((keyword) =>
          contextAfterClose.includes(keyword),
        ) &&
        !hasSectionSeparator
      ) {
        hasStrongEmitterIndicator = true;
      }
      const hasStrongReceiverIndicator = strongReceiverKeywords.some(
        (keyword) => contextLower.includes(keyword),
      );
