<script lang="ts">
  ...
  async function handleFileLoadError(fileId: number, filename: string) {
    try {
      ...
      if (!response.ok) {
        ...
        toast.error(`No se pudo cargar "${filename}": ${errorData.error || 'Error desconocido'}`, {
          duration: 5000,
        });
      }
    } catch (err) {
  ...
  function selectCandidate(fileId: number, candidateIndex: number) {
    ...
    toast.success(
      `Candidato seleccionado: ${candidate.invoiceType}-${candidate.pointOfSale}-${candidate.invoiceNumber}`
    );
  }
  ...
  <nav class="tabs">
    ...
    >
      ...
      {#if (pendingFilesStats?.pending || 0) + (pendingFilesStats?.failed || 0) > 0}
        <span class="badge"
          >{(pendingFilesStats?.pending || 0) + (pendingFilesStats?.failed || 0)}</span
        >
      {/if}
    </button>
    <button
      ...
      {:else}
        <div class="review-list">
          {#each pendingFilesToReview as file (file.id)}
            ...
            {@const excelData =
              matchInfo?.exactMatch || matchInfo?.bestMatch || matchInfo?.candidates?.[0] || null}
            ...
            {@const cuitMatch =
              file.extractedCuit && excelData?.cuit && file.extractedCuit === excelData.cuit}
            {@const dateMatch =
              file.extractedDate &&
              excelData?.issueDate &&
              file.extractedDate === excelData.issueDate}
            {@const typeMatch =
              file.extractedType &&
              excelData?.invoiceType &&
              file.extractedType === excelData.invoiceType}
            {@const posMatch =
              file.extractedPointOfSale != null &&
              excelData?.pointOfSale != null &&
              file.extractedPointOfSale === excelData.pointOfSale}
            {@const numMatch =
              file.extractedInvoiceNumber != null &&
              excelData?.invoiceNumber != null &&
              file.extractedInvoiceNumber === excelData.invoiceNumber}
            {@const totalMatch =
              file.extractedTotal != null &&
              excelData?.total != null &&
              Math.abs(file.extractedTotal - excelData.total) < 0.01}
                  ...
                  <div class="comparison-section">
                    ...
                    <div class="confidence-metrics">
                      <div
                        ...
                        class:low={ocrConfidence < 50}
                        class:medium={ocrConfidence >= 50 && ocrConfidence < 80}
                      ...
                      {#if excelData}
                        <div
                          class="confidence-bar fisco"
                          class:low={matchScore < 50}
                          class:medium={matchScore >= 50 && matchScore < 80}
                          class:high={matchScore >= 80}
                        >
                      ...
                      <div class="candidates-selector">
                        <label>
                          <span class="selector-label"
                            >{matchInfo.candidates.length} candidatos encontrados:</span
                          >
                          <select
                            onchange={(e) =>
                              selectCandidate(file.id, parseInt(e.currentTarget.value))}
                          >
                            {#each matchInfo.candidates as candidate, idx}
                              <option value={idx}>
                                {candidate.invoiceType}-{String(candidate.pointOfSale).padStart(
                                  4,
                                  '0'
                                )}-{String(candidate.invoiceNumber).padStart(8, '0')} ({candidate.matchScore}%
                                match)
                              </option>
                            {/each}
                          </select>
                    ...
                    <table class="comparison-table">
                      ...
                      <tbody>
                        <!-- CUIT -->
                        <tr
                          class:match={cuitMatch}
                          class:no-match={file.extractedCuit && excelData?.cuit && !cuitMatch}
                          class:missing={!file.extractedCuit}
                        >
                          ...
                          <td class="status-cell">
                            ...
                            {:else}
                              <span
                                ...
                                title="No coincide: PDF={file.extractedCuit}, Excel={excelData.cuit}"
                                >⚠</span
                              >
                            {/if}
                          </td>
                        ...
                        <!-- Fecha -->
                        <tr
                          class:match={dateMatch}
                          class:no-match={file.extractedDate && excelData?.issueDate && !dateMatch}
                        ...
                        >
                          ...
                          <td class="status-cell">
                            ...
                            {:else}
                              <span
                                class="status-icon error"
                                title="No coincide: PDF={file.extractedDate}, Excel={excelData.issueDate}"
                                >⚠</span
                              >
                            {/if}
                          </td>
                        ...
                        <!-- Punto de Venta -->
                        <tr
                          class:match={posMatch}
                          class:no-match={file.extractedPointOfSale != null &&
                            excelData?.pointOfSale != null &&
                            !posMatch}
                        ...
                        >
                          ...
                          <td class="status-cell">
                            ...
                            {:else}
                              <span
                                class="status-icon error"
                                title="No coincide: PDF={file.extractedPointOfSale}, Excel={excelData.pointOfSale}"
                                >⚠</span
                              >
                            {/if}
                          </td>
                        ...
                        <!-- Número -->
                        <tr
                          class:match={numMatch}
                          class:no-match={file.extractedInvoiceNumber != null &&
                            excelData?.invoiceNumber != null &&
                            !numMatch}
                        ...
                        >
                          ...
                          <td class="status-cell">
                            ...
                            {:else}
                              <span
                                class="status-icon error"
                                title="No coincide: PDF={file.extractedInvoiceNumber}, Excel={excelData.invoiceNumber}"
                                >⚠</span
                              >
                            {/if}
                          </td>
                        ...
                        <!-- Total -->
                        <tr
                          class:match={totalMatch}
                          class:no-match={file.extractedTotal != null &&
                            excelData?.total != null &&
                            !totalMatch}
                        ...
                        >
                          ...
                          <td class="status-cell">
                            ...
                            {:else}
                              <span
                                class="status-icon error"
                                title="Diferencia: PDF=${file.extractedTotal}, Excel=${excelData.total}"
                                >⚠</span
                              >
                            {/if}
                          </td>
                  ...
                  <div class="review-actions">
                    {#if excelData}
                      <button
                        ...
                        onclick={() => applyExcelData(file.id)}
                        title="Usar datos del Excel"
                      >
                        📋 Aplicar Excel
                      </button>
                    {/if}
                    <button
                      ...
                      title="Volver a procesar con el algoritmo actual"
                    >
                      🔄 Reprocesar
                    </button>
