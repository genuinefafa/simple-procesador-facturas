...
export const GET: RequestHandler = async ({ params }) => {
  ...
  try {
    ...
    if (ext === "heic" || ext === "heif") {
      console.info(
        `   🔄 [FILE-SERVER] Convirtiendo HEIC a JPEG para preview...`,
      );
      try {
        ...
        console.info(
          `   ✅ [FILE-SERVER] HEIC convertido a JPEG (${fileBuffer.length} bytes)`,
        );
    ...
    console.info(
      `✅ [FILE-SERVER] Sirviendo: ${displayFilename} (${fileBuffer.length} bytes)`,
    );
